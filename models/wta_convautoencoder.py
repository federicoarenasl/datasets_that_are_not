...
class WTALifetimeSparseConvAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        top_activations, _ = torch.topk(encoded.view(encoded.size(0), -1), num_activations, dim=1)
        threshold = top_activations[:, -1].unsqueeze(1).unsqueeze(2).unsqueeze(3)
        encoded_wta = torch.where(encoded >= threshold, encoded, torch.zeros_like(encoded))
